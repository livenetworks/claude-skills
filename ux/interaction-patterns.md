...
> Visual spec → `ui/components/search.md`
...
User types
  → Filter data instantly on every keystroke (client-side from IndexedDB/DOM)
...
- **Instant on keyup** — when data is client-side (IndexedDB/DOM), filter on every keystroke. No debounce needed — `Array.filter()` on cached data is synchronous and instant
...
> Visual spec → `ui/components/data-table.md`
...
> Visual spec → `ui/components/empty-state.md`
