...
**Two layers in this document:**
- **§1–§8** — ln-acme implementation rules. Reference Inter, Tabler, SCSS mixins, specific token values. Directly prescriptive for this library.
- **§9+** — Universal design principles. Implementation-agnostic. Apply to any stack or framework that uses ln-acme or otherwise.
...
| Inputs, selects | Border only, no decorative shadow | They sit on the surface |
| Focused inputs | Focus indicator (see §10) | Focus rings are functional signals, not elevation |
...
| Focus | Visible and consistent — type depends on context, see §10 |
...
**Exception — functional indicators:** Navigation bar indicators and focus rings may change in opacity on hover. This is not decoration — it is communicating the availability of a state. Opacity change on a functional element is permitted; transform or shadow appearing on the interactive element itself is not.
...
### Sizing — brief note *[ln-acme]*

Modal width is set in SCSS per modal instance using size mixins. The size choice is a content decision: a short confirmation uses the smallest size; a multi-field form uses a larger one. Do not choose size for aesthetic reasons — let the content define the footprint.
